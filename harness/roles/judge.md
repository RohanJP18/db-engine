...
### Step 8: Create PR or Request Rework

If **ALL PASS** - Create a Pull Request:
```bash
gh pr create --base main --head agent/judge --title "Iteration N: [summary of completed tasks]" --body "$(cat <<'EOF'
## Summary
- Tasks evaluated: X
- All passed: YES

## Completed Tasks
- [DB-XXX] Task description
- [DB-YYY] Task description

## Verdict
All acceptance criteria met. Ready for merge.

---
*Auto-generated by Judge agent*
EOF
)"
```

Then notify human: "PR created for iteration N. Please review and merge: [PR URL]"

If **ANY FAIL** - Request Rework:
- Do NOT create a PR
- Workers will pull your changes to see REWORK feedback in TASKS.md
- Continue monitoring for re-submissions
- Once workers fix and mark DONE again, re-evaluate
